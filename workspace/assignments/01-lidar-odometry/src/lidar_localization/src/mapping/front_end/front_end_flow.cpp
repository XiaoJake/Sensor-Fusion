...
FrontEndFlow::FrontEndFlow(ros::NodeHandle& nh) {
    ...
    lidar_to_base_ptr_ = std::make_shared<TFListener>(nh, "base_link", "laser_link");

    cloud_pub_ptr_              = std::make_shared<CloudPublisher>(nh, "current_scan", "/base_link", 10);
    reflector_points_pub_ptr    = std::make_shared<CloudPublisher>(nh, "reflector_points", "/base_link", 10);
    ...
    front_end_path_pub_ptr_     = std::make_shared<PathPublisher>(nh, "front_end_path", "/map", 10);
    reflector_loc_path_pub_ptr_ = std::make_shared<PathPublisher>(nh, "reflector_loc_path", "/map", 10);
    front_end_tf_pub_ptr_       = std::make_shared<TFBroadCaster>("/map", "/base_link");
...
bool FrontEndFlow::InitCalibration() {
    ...
    if (!calibration_received) {
        if (lidar_to_imu_ptr_->LookupData(lidar_to_imu_) &&
            lidar_to_base_ptr_->LookupData(lidar_to_base_)) {
...
bool FrontEndFlow::UpdateReflectorOdometry(){
    front_end_ptr_->UpdateReflectorLoc(current_cloud_data_.cloudi_ptr, reflector_odometry_);
    reflector_odometry_ *= lidar_to_base_;
    return true;
}
...
bool FrontEndFlow::UpdateLaserOdometry() {
    ...
    if (!front_end_pose_inited) {
        ...
        if(!use_reflector_)
            ...
            front_end_ptr_->SetInitPose(reflector_odometry_);
...
bool FrontEndFlow::PublishData() {
    ...
    front_end_path_pub_ptr_->Publish(laser_odometry_);
    reflector_loc_path_pub_ptr_->Publish(reflector_odometry_);

    front_end_tf_pub_ptr_->SendTransform(laser_odometry_, current_imu_data_.time);
    ...
    reflector_points_pub_ptr->Publish(current_reflector_ptr);
