...
bool FrontEnd::UpdateReflectorLoc(CloudData::CLOUDI_PTR points_raw, Eigen::Matrix4f& reflector_pose) {
    ...
    CloudData::CLOUD_PTR reflector_points(new CloudData::CLOUD);
    ...
    pcl::EuclideanClusterExtraction<CloudData::POINT> ec;//欧式聚类对象
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
    {
        ...
        //LOG_EVERY_N(INFO,1) << "PointCloud representing the Cluster: " << cloud_cluster->points.size () << " data points.\n";
    ...
    if(centers.size() = 2)
    {
        Eigen::Vector2f r1 = centers[0];
        Eigen::Vector2f r2 = centers[1];
        float d = (r2 - r1).norm();

        double angle = pcl::getAngle3D(r1, r1, true);
